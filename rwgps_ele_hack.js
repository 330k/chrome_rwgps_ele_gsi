(function () {
  const buttons = [];
  {
    ...
    let func_str = '(e=>{let t="__NOT_EXISTS__",l=[{tilename:"dem5a_png",zoom:15},{tilename:"dem5b_png",zoom:15},{tilename:"dem5c_png",zoom:15},{tilename:"dem_png",zoom:14}],n="__330k_ele_gsi",a="fetchElevations_org",i="interval_timer";e[n]=e[n]??{};let o=e[n];function r(e,t,l=1e3,n=2592e6){let a="_expire_on",i=new Map,o=null,r=!1;return caches.open(e).then(e=>{o=e,r=!0}),this.fetch=async e=>{let c=null,s=!1,f=!1,g=Date.now(),h=g+n;if(!r)throw new Exception("Cache API not ready");if(i.has(e)){let p=i.get(e);g>p.expire_on?s=!0:(i.delete(e),i.set(e,p),c=p.data)}else{let d=await o.match(e);void 0===d||null===d.headers.get(a)||g>Number.parseInt(d.headers.get(a))?s=!0:(c=await t(d),h=Number.parseInt(d.headers.get(a)),f=!0)}if(s){let $=await fetch(e),E=$.clone(),m=new Headers(E.headers);m.append(a,h);let u=await E.blob();await o.put(e,new Response(u,{status:E.status,statusText:E.statusText,headers:m})),c=await t($),f=!0}return f&&(i.set(e,{data:c,expire_on:h}),i.length>l&&i.delete(i.keys().next().value)),c},this}o[a]=o[a]??Routes.activeMap.fetchElevations;let c=(()=>{let e=document.createElement("canvas"),l=e.getContext("2d"),n=document.createElement("img");return e.width=256,e.height=256,async e=>{if(!e.ok)return t;{let a=await e.clone().blob(),i=await new Promise((e,t)=>{n.src=URL.createObjectURL(a),n.onload=()=>{e(n)}});return l.drawImage(i,0,0,256,256),URL.revokeObjectURL(n.src),l.getImageData(0,0,256,256).data}}})(),s=new r("__330k_gsi_ele_tile",c);function f(e,t,l){let n=(t+180)/360*Math.pow(2,l),a=(1-Math.log(Math.tan(e*Math.PI/180)+1/Math.cos(e*Math.PI/180))/Math.PI)/2*Math.pow(2,l);return{tile_x:Math.floor(n),tile_y:Math.floor(a),pixel_x:(n-Math.floor(n))*256,pixel_y:(a-Math.floor(a))*256}}async function g(e,t){let n=null;for(let a of l)if(null!==(n=await h(e,t,a.tilename,a.zoom)))break;return n}async function h(e,l,n,a){let i=f(e,l,a),o=await s.fetch("https://cyberjapandata.gsi.go.jp/xyz/"+n+"/"+a+"/"+i.tile_x+"/"+i.tile_y+".png");if(o===t)return null;{let r=4*(256*Math.floor(i.pixel_y)+Math.floor(i.pixel_x)),c=65536*o[r]+256*o[r+1]+o[r+2];return c<8388608?.01*c:8388608===c?null:(c-16777216)*.01}}async function p(e,t){let l=[];for(let n of e)n.fetchingEle=!0,n.fetchingEleCompleted=!1;for(let i of e)try{let r=await g(i.point.lat,i.point.lng);if(Number.isFinite(r))i.ele=r,i.fetchingEleCompleted=!0;else throw Error("TILE ELE ERROR")}catch(c){l.push(i),console.log(c)}if(l.length>0)try{for(let s of(await new Promise(e=>{o[a](l,e)}),l))Number.isFinite(s.ele)?s.fetchingEleCompleted=!0:console.log("Fallback Error");console.log(l.map(e=>e.ele))}catch(f){console.log("Fallback Error")}for(let h of(console.log({requested:e.length,completed:e.filter(e=>e.fetchingEleCompleted).length}),e))Number.isFinite(h.ele)&&(delete h.fetchingEle,delete h.fetchingEleCompleted);0===e.filter(e=>!Number.isFinite(e.ele)).length&&t()}Routes.activeMap.fetchElevations===o[a]?(Routes.activeMap.fetchElevations=p,document.getElementById("%%TEMPLATE_BUTTON_ID%%").style.borderColor="pink"):(Routes.activeMap.fetchElevations=o[a],document.getElementById("%%TEMPLATE_BUTTON_ID%%").style.borderColor="transparent"),o[i]=o[i]??setInterval(()=>{let e=document.getElementById("%%TEMPLATE_MESSAGE_ID%%"),t=Routes.activeMap.activeRoute.trackPoints();t.filter(e=>e.fetchingEle).length?(e.style.backgroundColor="red",e.style.width=100*t.filter(e=>e.fetchingEleCompleted).length/t.filter(e=>e.fetchingEle).length+"%"):e.style.backgroundColor="transparent"},200)})(window);';
